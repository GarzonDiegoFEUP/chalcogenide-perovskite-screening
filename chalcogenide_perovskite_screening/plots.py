...
from chalcogenide_perovskite_screening.config import FIGURES_DIR, PROCESSED_DATA_DIR, RAW_DATA_DIR, RESULTS_DIR, INTERIM_DATA_DIR
...
def colormap_radii(df: pd.DataFrame, exp_df: pd.DataFrame, clf_proba: Any | None = None, t_sisso: bool = False) -> None:
    ...
    if clf_proba is None:
        from chalcogenide_perovskite_screening.modeling.train import train_platt_scaling
...
def plot_matrix(df_out: pd.DataFrame, df_crystal: pd.DataFrame, anion: str = 'S', parameter: str = 'Eg', clf_proba: Any | None = None) -> None:
    ...
    from chalcogenide_perovskite_screening.config import FIGURES_DIR

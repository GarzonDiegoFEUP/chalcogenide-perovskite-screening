...
from chalcogenide_perovskite_screening.synthesis_planning.reactions import Reaction
from chalcogenide_perovskite_screening.synthesis_planning.synthesis_pathways import get_inverse_hull_energy
from chalcogenide_perovskite_screening.config import SYNTHESIS_DATA_DIR, SYNTHESIS_FIG_DIR

'''layout of the interfacial reaction convex hull'''

plotly_layout_file = str(SYNTHESIS_DATA_DIR / "plot_layout" / "plotly_pd_layouts.json")

with open(plotly_layout_file, "r") as f:
...
class Inter_PDPlotter(PDPlotter):
    ...
    def show(self, to_show=True, *args, **kwargs):
        """
        ...
        if not filename:
            ...
            directory = str(SYNTHESIS_FIG_DIR / "phase_diagrams")
        filename_html = directory + "/html/" + filename
        filename_png = directory + "/png/" + filename.split(".")[0] + ".png"
